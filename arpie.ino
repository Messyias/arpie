...
//                                   0123456789012345
#define ARP_SCALE_CHROMATIC   (int)0b0000111111111111
#define ARP_SCALE_IONIAN      (int)0b0000101011010101
#define ARP_SCALE_DORIAN      (int)0b0000101101010110
#define ARP_SCALE_PHRYGIAN    (int)0b0000110101011010
#define ARP_SCALE_LYDIAN      (int)0b0000101010110101
#define ARP_SCALE_MIXOLYDIAN  (int)0b0000101011010110
#define ARP_SCALE_AEOLIAN     (int)0b0000101101011010
#define ARP_SCALE_LOCRIAN     (int)0b0000110101101010
...
void arpBuildSequence()
{
  ...
  int i, j;
  arpSequenceLength = 0;
  byte mode = (arpType == ARP_TYPE_GATED_CHORDS)? ARP_INSERT_OFF:arpInsertMode;
  switch(mode)
  {
  case ARP_INSERT_OFF:
...
void arpStopAllNotesInChord(unsigned long milliseconds)
{
  for(int i = 0;i < arpSequenceLength; ++i)
  {
    byte note = ARP_GET_NOTE(arpSequence[i]);
    if(note > 0)
...
{  
  ...
  {                 
    ...
    {
      if(ARP_TYPE_GATED_CHORDS == arpType)
      {
        for(i = 0;i < arpSequenceLength; ++i)
        {
          byte note = ARP_GET_NOTE(arpSequence[i]);
          byte velocity = arpVelocityMode? arpVelocity : arpChord[i];        
          if(note > 0)          
            midiWrite(MIDI_MK_NOTE, note, velocity, 2, milliseconds);           
        }        
        arpStopNote = ALL_NOTES_IN_CHORD;
      }
      else
      {
